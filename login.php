...
//         // tidak pakai password_hash untuk verifikasi
...
//         else {
//             var_dump($row);
//             echo "<script>alert('Invalid login credentials ke 1.');</script>";
//         }
//     } 
//     else {
//         var_dump($result);
//         echo "<script>alert('Invalid login credentials ke 2.');</script>";
//     }
// }

// Jika user sudah login → langsung ke index
if (isset($_SESSION['username'])) {
    header("Location: index.php");
    exit();
}

// Jika request AJAX POST → proses login
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    exit; // hentikan script setelah response JSON
}
...
<body class="flex justify-center items-center font-Mons">
    ...
    <div class="bg-white/40 backdrop-blur-md w-full max-w-sm p-8 space-y-6 bg-card-bg rounded-3xl">
        ...
        <div class="flex flex-col items-center space-y-2">
            ...
            <h1 class="text-3xl font-extrabold tracking-tight text-center">Welcome back!</h1>
            <p class="text-sm text-center font-z">
               Your assignments are waiting! It's time to get back to focusing and being productive.
            </p>
        </div>
        ...
        <div class="space-y-4">
            <!-- Input Email -->
            <div>
                <input type="text" id="email" name="login" placeholder="Enter your email or username"
            ...
            <div class="flex justify-between items-center text-sm">
                <div class="flex items-center hover:text-blue-500 transition ease-in-out duration-200">
                    <input class="appearance-none border-1 w-4 h-4 rounded-sm bg-transparent border-gray-500/40 hover:border-blue-500 checked:bg-blue-500 transition ease-in-out duration-200" id="remember-me" type="checkbox" >
                    <label for="remember-me" class="ml-2">Remember me</label>
                </div>
                <a href="#" onclick="showCustomMessage('Fitur lupa kata sandi sedang dalam pengembangan.', 'bg-yellow-600')" class=" hover:text-blue-400 transition duration-150">Forgot password?</a>
            </div>
        ...
        <p class="text-center text-sm  pt-4">
            ...
            <a href="signup.php"  class="font-bold text-white hover:text-blue-500 transition duration-150">Sign up now</a>
        </p>
<!-- onclick="showCustomMessage('Anda diarahkan ke halaman pendaftaran. Mari mulai terorganisir!', 'bg-indigo-600')" -->
    ...
    <script>
        ...
        //  DIUBAH JADI YANG DIBAWAH
        //         function handleLogin() {
        //     const email = document.getElementById('email').value;
        //     const password = document.getElementById('password').value;

        //     if (email === '' || password === '') {
        //         showCustomMessage('Email / Username dan Kata Sandi wajib diisi.', 'bg-red-600');
        //         return;
        //     }
        ...
        //     } else {
        //         showCustomMessage('Gagal Masuk. Email / Username atau kata sandi tidak valid.', 'bg-red-600');
        //     }
        // }
    ...
    if (login === '' || password === '') {
        showCustomMessage('Harap isi semua data', 'bg-red-600');
        return;
    }
    ...
    .then(data => {
        if (data.status === 'success') {
            showCustomMessage('Login Berhasil!', 'bg-green-600');
            setTimeout(() => {
                window.location.href = 'index.php';
            }, 700);
        } else {
            showCustomMessage(data.message, 'bg-red-600');
        }
    });
});
